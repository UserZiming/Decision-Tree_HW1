...
# run standard perceptron and get predictions on test dataset
w = perceptron.run(train_df, T, lr)
after_test_df = perceptron.predict(test_df_nolabel, w)

# compute average prediction error
average_prediction_error = sum(abs(after_test_df['prediction'] - test_df['label'])) / 2 / len(test_df)

print('Learned weight vectors: ', w)
